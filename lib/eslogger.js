var
    ...
    , defaultFilters = {
        'req' : require('./filters/req').get_req_props
    }
;

var ESLogger = function (options) {
    ...
    if (self.options.filters.length >= 0){
        self.options.filters = _.map(self.options.filters,function(f){
            if (f && _.isString(f)){
                var fn = defaultFilters[f];
                if (!fn){
                    throw new Error('ESLogger built-in filter "' + f + '" doesn\'t exist');
                }
                return defaultFilters[f];
            }
            if (f && !_.isFunction(f)){
                throw new Error('ESLogger filters must be a string or object, not "' + typeof f + '"');
            }
            return f;
        });
    }
